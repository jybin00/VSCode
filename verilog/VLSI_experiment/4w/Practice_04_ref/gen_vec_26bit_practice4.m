...
input_bit = 26;    % input  vector bitwidth
output_bit = 27;   % output vector bitwidth
...
A_in_hex = fopen('./a_input_26b.txt', 'w');
for k = 1:N
    pr_A = int_A(k);
    fprintf(A_in_hex,'%x \n', pr_A);
end
...
B_in_hex = fopen('./b_input_26b.txt', 'w');
for l = 1:N
    pr_B = int_B(l);
    fprintf(B_in_hex,'%x \n', pr_B);
end
...
int_Out = int_A + int_B;
Out_in_hex = fopen('./sum_output_27b.txt', 'w');
for m = 1:N
    pr_Out = int_Out(m);
